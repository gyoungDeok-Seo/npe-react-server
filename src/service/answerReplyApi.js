export const replyListApi = async (qnaId) => {
    const response = await fetch(`http://localhost:10000/answers/api/list?id=${qnaId}`, {
        method: "GET",
        credentials: "include",
        headers: {
...
export const replyLikeApi = async (data) => {
    const response = await fetch("http://localhost:10000/replies/api/reply-like", {
        method: "POST",
        credentials: "include",
        headers: {
            "Content-Type": "application/json",
...
export const checkTheComment = async (replyContent) => {
    const response = await fetch(`http://localhost:10001/ai/api/replyFilters/?reply-content=${replyContent}`);
