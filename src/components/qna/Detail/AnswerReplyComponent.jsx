...
function AnswerReplyComponent({ reply, index }) {
    const type = "댓글";
    ...
    const [replyModifyValue, setReplyModifyValue] = useState(reply.replyContent);
    ...
    const handlerClickReplyLike = () => {
        setIsReplyLike((isReplyLike) => !isReplyLike);
    };
        ...
        <>
            <AnswerReplyTopBox key={index}>
                <AnswerReplyWriterInfoLink to={""}>
                    <AnswerReplyWriterProfileImg src={reply.replyProfileUrl} />
                    <div>
                        <AnswerReplyWriterNameBox>
                            <AnswerReplyWriterName>{reply.replyWriterName}</AnswerReplyWriterName>
                            <AnswerReplyWriterDisplayBox>
                                <AnswerReplyWriterDisplay>작성자</AnswerReplyWriterDisplay>
                            </AnswerReplyWriterDisplayBox>
                        </AnswerReplyWriterNameBox>
                        <AnswerReplyWriterPositionTime>
                            <span>{reply.replyWriterPosition} • </span>
                ...
                <AnswerReplyContentBox>
                    {isReplyModify ? (
                        <AnswerReplyContentModifyBox>
                            <AnswerReplyContentModifyTextarea onChange={handlerChangeValue} style={{ height: "24px" }} placeholder="댓글을 남겨주세요">
                                {reply.replyContent}
                            </AnswerReplyContentModifyTextarea>
                            <AnswerReplyContentModifyBtn onClick={handlerClickModifyDoneBtn} isWrite={replyModifyValue} disabled={!replyModifyValue}>
                                수정
                            </AnswerReplyContentModifyBtn>
                        </AnswerReplyContentModifyBox>
                    ) : (
                        <AnswerReplyContentItem>
                            <p style={{ overflowWrap: "anywhere", whiteSpace: "pre-wrap" }}>{reply.replyContent}</p>
                        </AnswerReplyContentItem>
                    )}
                </AnswerReplyContentBox>
                {!isReplyModify && (
                    <AnswerReplyActionBox>
                        <AnswerReplyActionBtnBox>
                            <AnswerReplyActionBtn type="button" onClick={handlerClickActionBtn}>
                                <AnswerReplyActionSvg width="16" height="16" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-label="옵션 더 보기">
                                    <g>
                                        <g id="style=outline">
                                            <path
                                                id="Vector (Stroke)"
                        ...
                        {isActionMenu && (
                            <AnswerReplyActionMenuBox>
                                {/* 다른사람이 보는 메뉴 */}
                                <AnswerReplyReportBtn type="button" onClick={handlerClickReplyReportBtn}>
                                    <AnswerReplyReportSvg width="20" height="20" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <g>
                                            <g id="style=outline">
                                                <path
                                                    id="Vector (Stroke)"
                                ...
                                <AnswerReplyModifyBtn onClick={handlerClickReplyModifyBtn}>
                                    <AnswerReplyModifySvg width="20" height="20" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <g>
                                            <g id="style=outline">
                                ...
                                <AnswerReplyDeleteBtn onClick={handlerClickReplyDeleteBtn}>
                                    <AnswerReplyDeleteSvg width="20" height="20" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <g>
                                            <g id="style=outline">
                                                <path
                                                    id="Vector (Stroke)"
            ...
            <AnswerReplyBottomBox>
                {reply.likePeople ? (
                    <AnswerReplyListBtn onClick={handlerClickPeopleView}>
                        <span>좋아요 {reply.likePeople.length}</span>
                    </AnswerReplyListBtn>
                ) : (
                    <></>
                )}
                <AnswerReplyLikeBtn onClick={handlerClickReplyLike}>
                    <AnswerReplyLikeSvg isLike={isReplyLike} width="16" height="16" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <g>
                            <g id="style=solid">
                                {isReplyLike ? (
                                    <path
            ...
            {replyReportModal && <ReportModal setModal={setReplyReportModal} type={type} />}
            {replyDeleteModal && <DeleteModal setModal={setReplyDeleteModal} />}
        </>
    );
}
