...
function AnswerReplyComponent({ reply, index }) {
  ...
  const handlerClickModifyDoneBtn = async () => {
    await updateReply(reply.id, replyModifyValue);
    // dispatch(setQnaDetailData(await readQnaDetail(reply.id)));
  ...
  const handlerClickReplyLike = async () => {
    const likeStatus = await snedReplyLike(reply?.id);
    setIsReplyLike(likeStatus.status);
  };
  ...
  return (
    <>
      ...
      {replyDeleteModal && (
        <DeleteModal setModal={setReplyDeleteModal} data={reply} />
      )}
    </>
  );
}
