...
const WriteOptionBtnSvg = styled.svg`
  ...
  transform: translate(0, 0)
    ${(props) => (props.writeModalOpen ? "rotate(180deg)" : "rotate(0deg)")}
...
function OnLoginNav() {
  ...
  const handleClickOutside = (event) => {
    ...
    if (
      writeOptionRef.current &&
      !writeOptionRef.current.contains(event.target) &&
      writeModalRef.current &&
      !writeModalRef.current.contains(event.target)
    ) {
