...
const SubmitProfileUpdateBtn = styled.button`
  ...
  &:not(:disabled):hover {
    background-color: var(
      --color-background-button-primary-bold-hover,
      #1e293b
    );
  }
  &:disabled {
...
const ProfileUpdateHeader = ({ member, navigate, setAvoidMistakesModal }) => {
  ...
  const createCareerFetch = async (careerInfo) => {
    const response = await fetch(
      `http://localhost:10000/members/api/create-career`,
      {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  const memberInfoModify = async () => {
    ...
    const response = await fetch(
      `http://localhost:10000/members/api/member-info/modify`,
      {
        method: "PATCH",
        credentials: "include", // 세션 쿠키를 포함하여 요청,
        headers: {
