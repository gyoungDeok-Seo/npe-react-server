...
const ProfileUpdateHeader = ({ navigate, setAvoidMistakesModal }) => {
    ...
    }, [profileUpdate, createSkills, location.pathname]);
    ...
    const handleUpdateProfile = () => {
        if (location.pathname.startsWith("/profiles/update")) {
            ...
            getMemberSkillsListFetch()
                .then((skillList) => {
                    const memberSkillsSet = new Set(skillList.map((skill) => skill.skillName));
                    const createSkillsSet = new Set(createSkills.skills.map((skill) => skill.skillName));
                    const filteredCreateSkills = createSkills.skills.filter((skill) => !memberSkillsSet.has(skill.skillName));
                    const filteredRemoveSkills = skillList.filter((skill) => !createSkillsSet.has(skill.skillName));

                    return { addList: filteredCreateSkills, removeList: filteredRemoveSkills };
                })
                .then((modifyInfo) => {
                    memberSkillListModifyFetch(modifyInfo);

                    const updateSkills = [...createSkills.skills, ...modifyInfo.addList, ...createSkills.skills.filter((skill) => !modifyInfo.removeList.some((removeSkill) => removeSkill.skillName === skill.skillName))];
