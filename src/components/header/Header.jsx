...
const ProfileUpdateHeader = ({ navigate }) => {
  const handleUpdateProfile = () => {
    navigate("/profiles");
  };

  return (
    <nav className="profile-update-header">
      <Link to="/profiles" className="exit-profile-update-btn">
        <svg
          width="24"
          height="24"
          strokeWidth="0"
      ...
      <p className="profile-update-header-center-space"></p>
      <button
        type="button"
        className="submit-profile-update-btn"
        onClick={handleUpdateProfile}
      >
        <span className="submit-profile-update-btn-text">완료</span>
      </button>
    </nav>
  );
};

const NonLoginHeader = ({ setSearch }) => {
        ...
        <div className="right-box">
          ...
          <div>
            <button className="join-btn">
              <span className="join-btn-text">회원가입</span>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      {path.startsWith("/profiles/") ? (
        <ProfileUpdateHeader navigate={navigate} />
      ) : (
        <NonLoginHeader setSearch={setSearch} />
      )}
    </>
  );
}
