...
function SkillSearchInput() {
    ...
    const handleSearch = (item) => {
        console.log(item);
        const checkSkill = createSkills.skills.some((skill) => skill.skillName === item.skillName);
    ...
    return (
        <>
            <CareerSkillModalInputBox onFocus={handleFocus} onBlur={handleBlur} isFocused={isFocused}>
                <CareerSkillModalInput type="text" placeholder="스킬을 검색해 보세요." autoComplete="off" value={inputValue} onChange={searchSkills} />
                {isFocused ? (
                ...
                {showBox && (
                    <SkillList>
                        {search.length > 0 ? (
                            <>
                                {search.map((item) => (
                                    <SkillItem onClick={() => handleSearch(item)}>{item.skillName}</SkillItem>
                                ))}
                            </>
                        ) : (
