...
function CareerPeriod() {
    ...
    const [isWorking, setIsWorking] = useState(createCareer?.careerEnd !== null);
    const [endDateDisabled, setEndDateDisabled] = useState(!isWorking);

    // useEffect(() => {
    //     setIsWorking(targetData?.careerEnd !== null);
    //     setEndDateDisabled(!isWorking);
    //     dispatch(setCareerEnd(""));
    // }, []);

    const handleStartChange = (e) => {
        const newStartYear = e.target.value;
        const newCareerStart = `${newStartYear}-${createCareer.careerStart.split("-")[1] || ""}`;
    ...
    const handleEndYearChange = (e) => {
        ...
        const newCareerEnd = `${newEndYear}-${createCareer.careerEnd.split("-")[1] || ""}`;
        dispatch(setCareerEnd(newCareerEnd));
    };

    const handleEndMonthChange = (e) => {
        const newEndMonth = e.target.value;
        const newCareerEnd = `${createCareer.careerEnd.split("-")[0] || ""}-${newEndMonth}`;
        dispatch(setCareerEnd(newCareerEnd));
    };
    ...
    const handleIsWorking = () => {
        ...
        if (!isWorking) {
            ...
            dispatch(setCareerEnd("9999-99-99"));
        } else {
            setEndDateDisabled(false);
            dispatch(setCareerEnd(""));
        }
    ...
    return (
        <>
            <CareerSelectBox>
                ...
                <CareerPeriodSelectBox>
                    <CareerPeriodSelectInner>
                        <CareerPeriodSelect value={createCareer.careerStart.split("-")[0]} onChange={handleStartChange}>
                        ...
                        <CareerPeriodSelect value={createCareer.careerStart.split("-")[1]} onChange={handleStartMonthChange}>
                            <option disabled="" value="-1">
                                월
                            </option>
                    ...
                    <CareerPeriodSelectBox2>
                        {isWorking ? (
                            ...
                            <CareerPeriodSelectInner2>
                                <CareerPeriodSelect disabled={endDateDisabled} value={createCareer?.careerEnd?.split("-")[0]} onChange={handleEndYearChange}>
                                    <option disabled="" value="-1">
                                        종료연도
                                    </option>
                                    <option value="2024">2024년</option>
                                ...
                                <CareerPeriodSelect disabled={endDateDisabled} value={createCareer.careerEnd?.split("-")[1]} onChange={handleEndMonthChange}>
                                    <option disabled="" value="-1">
                                        월
                                    </option>
            ...
            <CareerCreateCheckboxBox>
                <CareerCreateCheckbox type="checkbox" id="is-current-career" onChange={handleIsWorking} checked={isWorking} />
