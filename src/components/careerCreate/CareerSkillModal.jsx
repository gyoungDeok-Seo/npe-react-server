...
const CareerSkillModalSkillBtn = styled.button`
    ...
    padding-right: 0.75rem;
    --tw-bg-opacity: 1;
    background-color: ${(props) => (props.selected ? "#334155" : "rgb(255 255 255)")};
    --tw-border-opacity: 1;
...
const CareerSkillModal = ({ setCareerSkillModal }) => {
    ...
    const [selectedSkills, setSelectedSkills] = useState(createCareer.careerSkills || []);
    useEffect(() => {
        const searchSkillsFetch = async (keyword) => {
            const response = await fetch(`http://localhost:10000/members/api/skillSearch?keyword=${keyword}`);
    ...
    const handleSearch = (skill) => {
        const checkSkill = createCareer.careerSkills.some((item) => item.skillName === skill.skillName);

        !checkSkill && setSelectedSkills((prevSelectedSkills) => (prevSelectedSkills.find((item) => item.skillId === skill.skillId) ? prevSelectedSkills.filter((item) => item.skillId !== skill.skillId) : [...prevSelectedSkills, skill]));
    ...
    const handleSkillClick = (skill) => {
        setSelectedSkills((prevSelectedSkills) => (prevSelectedSkills.find((item) => item.skillId === skill.skillId) ? prevSelectedSkills.filter((item) => item.skillId !== skill.skillId) : [...prevSelectedSkills, skill]));
    };
                ...
                >
                    <CareerSkillModalBox>
                        <CareerSkillModalContentBox>
                            <CareerSkillModalTitle>내 스킬을 선택해 주세요.</CareerSkillModalTitle>
                            {selectedSkills.length !== 0 && (
                                <CareerSkillModalCraeteItemBox>
                                    {selectedSkills.map((item) => (
                                        <CareerSkillModalCraeteItem key={item.skillId} type="button" onClick={() => handleSkillClick(item)}>
                                            <CareerSkillModalCraeteItemText>{item.skillName}</CareerSkillModalCraeteItemText>
                                            <CareerSkillModalCraeteItemSvgBox role="presentation">
                                                <CareerSkillModalCraeteItemSvg width="16" height="16" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <g>
                                                        <g id="style=solid">
                                                            <path id="Vector (Stroke)" fillRule="evenodd" clipRule="evenodd" d="M5.29289 5.29289C5.68342 4.90237 6.31658 4.90237 6.70711 5.29289L12 10.5858L17.2929 5.29289C17.6834 4.90237 18.3166 4.90237 18.7071 5.29289C19.0976 5.68342 19.0976 6.31658 18.7071 6.70711L13.4142 12L18.7071 17.2929C19.0976 17.6834 19.0976 18.3166 18.7071 18.7071C18.3166 19.0976 17.6834 19.0976 17.2929 18.7071L12 13.4142L6.70711 18.7071C6.31658 19.0976 5.68342 19.0976 5.29289 18.7071C4.90237 18.3166 4.90237 17.6834 5.29289 17.2929L10.5858 12L5.29289 6.70711C4.90237 6.31658 4.90237 5.68342 5.29289 5.29289Z"></path>
                                                        </g>
                                                    </g>
                            ...
                            <CareerSkillModalContent>
                                <CareerSkillModalInputBox onFocus={handleFocus} onBlur={handleBlur} isFocused={isFocused}>
                                    <CareerSkillModalInput type="text" placeholder="스킬을 검색해 보세요." value={inputValue} onChange={searchSkills} />
                                    {isFocused ? (
                                        <CareerSkillModalSearchBtn type="button" name="empty" onClick={handleClear}>
                                            <CareerSkillModalSearchSvg width="24" height="24" strokeWidth="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <g>
                                                    <g id="style=solid">
                                                        <path
                                                            id="Vector"
                                ...
                                <SkillListBox>
                                    {showBox && (
                                        <SkillList>
                                            {search.map((item) => (
                                                <SkillItem key={item.skillId} onClick={() => handleSearch(item)}>
                                                    {item.skillName}
                                                </SkillItem>
                                            ))}
                                        </SkillList>
                                ...
                                <div>
                                    ...
                                    <CareerSkillModalSkillList>
                                        {popularSkills.map((skill) => (
                                            <CareerSkillModalSkillBtn key={skill.skillId} type="button" onClick={() => handleSkillClick(skill)} selected={selectedSkills.find((item) => item.skillId === skill.skillId)}>
                                                <CareerSkillModalSkillBtnText selected={selectedSkills.find((item) => item.skillId === skill.skillId)}>{skill.skillName}</CareerSkillModalSkillBtnText>
                        ...
                        <CareerSkillModalBtnBox>
                            <CareerSkillModalCancelBtn type="button" onClick={handleCancelModal}>
                                <span>취소</span>
                            </CareerSkillModalCancelBtn>
                            <CareerSkillModalSubmitBtn type="button" onClick={handleSubmitModal}>
                                <span>확인</span>
