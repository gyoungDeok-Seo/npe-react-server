  ...
  useEffect(() => {
    const fetchMemberSkill = async () => {
      const response = await fetch(
        `http://localhost:10000/members/api/skill?memberId=${
          pathname.split("/")[2]
        }`,
        {
          method: "GET",
                  ...
                  <ProfileContentContainerInner>
                            ...
                            {memberCareer.careers.map((career, index) => (
                              <ContentItem ket={index}>
                                  ...
                                  {career.careerIndustries.length > 0 ? (
                                    <div>
                                      <ContentContentAnother>
                                        산업 분야:{" "}
                                        {career.careerIndustries.map(
                                          (careerIndustry, index) => (
                                            <>
                                              {careerIndustry.industryName}
                                              {index <
                                                career.careerIndustries.length -
                                                  1 && ", "}
                                            </>
                                          )
                                        )}
                                ...
                                <ContentContentEditBtn
                                  to={`/profiles/careers/update/${career.id}`}
                                >
                    ...
                    <ContentBox aria-label="교육">
                      ...
                      {isExistence ? (
                        ...
                        <NonExistent
                          partText="현재 혹은 이전에 다녔던 학교, 부트캠프 등 교육기관을 입력해 주세요."
                          part="교육"
