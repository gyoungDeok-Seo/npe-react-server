    ...
    <DetailContainer>
      <DetailBox>
        <DetailContents>
          ...
          <QnaDetailContent>
            위쪽에 있는 메뉴에 마우스를 올리면 전체 메뉴가 드랍 되는 형태의 헤더
            인데 <br />
            위쪽 메뉴에 마우스를 올리면 텍스트 컬러와 밑줄이 생기고 그 상태가
            유지 되면서 <br />
            하위 메뉴를 컨트롤 해야 하는데 마우스 커서를 하위 메뉴 쪽으로
            가져갔을때 위쪽 메뉴의 컬러와 <br />
          ...
          <ImgBox>
            {images.map((src, index) => (
              <ImageWrapper key={index}>
                <StyledImg
                  alt="이미지 설명"
                  srcSet={`${src}&w=1200&q=100 1x, ${src}&w=3840&q=100 2x`}
                  src={src}
                  decoding="async"
                />
              </ImageWrapper>
