...
import { answers } from "../../service/dummyData";
...
const QnaListLink = styled(Link)`
  ...
  background-color: var(
    --color-background-button-primary-tint-enabled,
    #f1f5f9
  );
...
function QnaDetailMainContainer() {
  ...
  // const { qnaDetailData: qnaData } = useSelector((state) => state.qnaDetail);
  ...
  const handleSendAnswer = async () => {
    ...
    setTextareaValue("");
    dispatch(setQnaDetailData(await readQnaDetail(qnaId)));
  };
  // useEffect(() => {
  //   console.log(data)
  // }, [data, dispatch]);
  useEffect(() => {
    dispatch(setQnaDetailData(data));
  }, [data, dispatch]);
        ...
        <DetailBox>
          <DetailContents>
            <QnaDetailHeaderBox>
              ...
              <div>
                <QnaTitle>{data?.questionTitle}</QnaTitle>
              </div>
              <WriteDateAndHits>
                <WriteDateAndHitsItem>
            ...
            <QnaDetailContent>{data?.questionContent}</QnaDetailContent>
            <ImgBox>
              {data?.fileList.map((item, index) => (
                <ImageWrapper key={index}>
                  <StyledImg
                    alt={item.fileName}
                    srcSet={`${item.filePath}&w=1200&q=100 1x, ${item.filePath}&w=3840&q=100 2x`}
                    src={item.filePath}
                    decoding="async"
                  />
                </ImageWrapper>
          ...
          <div>
            ...
            <AnswerListSector>
              <div>
                <AnswerListHeader>답변 {data?.answerList.length}</AnswerListHeader>
                <AnswerListContainer>
                  {data ? (
                    data?.answerList.map((answer, index) => (
                      <AnswerContentBox answer={answer} index={index} />
                    ))
                  ) : (
                    <></>
                  )}
